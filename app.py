...
st.set_page_config(page_title="Product Recommendation System", page_icon="favicon.ico", layout="wide")
# Load dataset
@st.cache_data
def load_data():
    try:
...
def collaborative_filtering_recommendations(data, selected_product_name, num_recommendations=3):
    try:
        ...
        selected_product_index = data[data['product_name'] == selected_product_name].index
        if len(selected_product_index) == 0:
        ...
        raise ValueError(f"An error occurred while generating recommendations: {e}")
